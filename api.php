...
function MFN_get_feed($include_content, $hasTags = array(), $hasNotTags = array(), $orTags = array(), $offset = 0, $limit = 30, $lang = 'all', $year = '', $fromyear = '', $fromdate = '', $todate = '')
{
    ...
    $query = "
SELECT post_date_gmt, p.post_title, tags, lang.meta_value lang, post_id, post_name" . ($include_content ? ', post_content, post_excerpt' : '' ) . "
FROM $wpdb->posts p
INNER JOIN $wpdb->postmeta lang
...
INNER JOIN (
 ...
 ";
    ...
    if (!empty($orTags) && count($orTags) > 0) {
        $query .= " AND (FALSE ";
        foreach ($orTags as $tag) {
            $query .= " OR FIND_IN_SET(%s, t.tag_slugs) > 0 ";
            array_push($params, $tag);
        }
        $query .= ") ";
    }
    ...
    if ($fromdate != "") {
        $query .= " AND DATE(p.post_date_gmt) >= %s ";
